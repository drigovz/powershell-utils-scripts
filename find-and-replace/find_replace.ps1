...
# replace folders
Get-ChildItem $currentLocation -R | Rename-Item -NewName { $_.Name -replace $oldValue, $newValue }
...
foreach ($file in Get-ChildItem $currentLocation -Recurse) {
    ...
    if (-not  ($file.Extension -eq '.ps1')) {
        ...
        if (-not $file.PSIsContainer) {
            ((Get-Content -Path $file.FullName -Raw) -replace $oldValue, $newValue) | Set-Content -Path $file.FullName
        }
    }
}
